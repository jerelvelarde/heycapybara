...
On first launch, OpenMuse opens a short setup window: a welcome, the two macOS permissions it uses, and where to find your companion. Accessibility is needed to record workflows across apps; Screen Recording is optional and used only for screenshots you explicitly attach. **Allow** asks macOS directly, and each permission shows as allowed once it is on in System Settings; **Settings → macOS permissions** offers the same **Open System Settings** button if a permission still isn’t granted. If OpenMuse is already listed but recording still fails, toggle its permission off and on and restart rather than adding a duplicate. Closing the window skips setup, and an existing install skips it too unless its saved settings say setup never finished. Replay it from **Settings → Desktop companion** or the menu-bar item. Setup hides the companion, which carries the always-visible recording indicator and its Stop button, so you can’t replay setup during a recording, and a recording can’t start until setup is finished. When setup ends, your companion appears.
