...
- Agent tools can open installed applications and display a pointer after a native approval dialog. The pointer targets a spot in a screenshot you attached: the model gives pixel coordinates in that image, and OpenMuse converts them to screen points. Screenshots are sized so Codex does not resize them, and a point is refused if the screenshot is no longer in memory (after a restart or 16 newer captures), is more than 10 minutes old, or its display has been disconnected, moved or changed resolution, and a point outside the image is rejected so the model can retry. Codex executes shell and file tasks in the selected workspace. Cross-app execution remains guided; desktop clicking and typing are not implemented.
