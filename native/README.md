...
- `--point x y`: validates finite Quartz coordinates against connected displays with `CGGetDisplaysWithPoint` (a display's top row counts, the row below its bottom doesn't) and displays a noninteractive red ring for 1.2 seconds. Does not activate, click, or type in the target app.
...
- `--keys key [modifier ...]`: one key name from the fixed list (US ANSI key positions) and up to four different modifiers (`command`, `shift`, `option`, `control`). It needs Accessibility and refuses while OpenMuse is the frontmost app. Where `--type` would refuse a secure text field, it also refuses a key that types a character (a letter, digit, punctuation key or `space`) with no modifiers, Shift or Option, and Command-V with or without other modifiers; Return, Tab, Delete, Escape, the arrows and other navigation keys, and other shortcuts still go through. It sends one key press with exactly those modifier flags, and no separate modifier key events, then emits `Keys pressed`.
